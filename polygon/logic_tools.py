...
def save_polygons(imframe, config, dst, scale):
    ...
    parser[str_polygons][str_holes] = codecs.encode(pickle.dumps(holes), 'base64').decode()
    ...
    with open(path, 'w') as f:
        ...
        parser.write(f)
    polygons = [polygon + [polygon[0]] for polygon in roi]
    if dst is None:
        dst = Path(imframe.path)
        dst = dst.parents[0] / (dst.stem+'_markup.json')
    dump_polygons(polygons, dst, scale)

def dump_polygons(polygons, dst, scale):
    data = []
    for polygon in polygons:
        data.append({"geometry": {"type": "Polygon", "coordinates": [[[x*scale, y*scale] for x, y in polygon]]}})
